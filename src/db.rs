...
use crate::buffers::msg_out_s::{Buf as MsgBuf, SIZE as MSG_SIZE};
use crate::helpers::{read_exact, Res};
...
pub fn fetch(path: &Path, mut count: u8) -> Result<(u8, u32, Vec<u8>), &'static str> {
    ...
    if file
        .seek(SeekFrom::End(i64::from(count) * NEG_MSG_SIZE))
        .is_err()
    {
        count = (file
            .seek(SeekFrom::End(0))
            .map_err(|_| "Failed to seek from end")?
            / MSG_SIZE_U64)
            .try_into()
            .map_err(|_| "Stream position > 255")?;
    ...
    let pos = file
        .stream_position()
        .map_err(|_| "Failed to read stream pos")?;
    if pos > MAX_FILE_SIZE {
        return Err("Too many messages in one file!");
    }

    let id = u32::try_from(pos / MSG_SIZE_U64).unwrap(); // can't fail
    if pos != (id * MSG_SIZE_U32).into() {
...
pub fn query(
    ...
    if count > MAX_FETCH_AMOUNT {
        count = MAX_FETCH_AMOUNT
    } // request too many, return max amount
    ...
    if id > db_len {
        return Ok(EMPTY_RESPONSE);
    } // outside of range, return nothing
    if forward && id >= db_len - 1 {
        return Ok(EMPTY_RESPONSE);
    } // nothing ahead of db_len
